...
/**
 * Same shape as MedicalHistoryFieldResource, plus the given student's answer
 * (looked up from a `medical_history_field_id => UserMedicalHistory` map)
 * merged into each field, including nested additional_fields.
 */
class StudentMedicalHistoryFieldResource extends JsonResource
{
     ...
     * @param  \Illuminate\Support\Collection  $answers  medical_history_field_id => UserMedicalHistory
     */
    ...
    public function toArray(Request $request): array
    {
        ...
        $additionalFields = $this->requiredByFields ? $this->requiredByFields->groupBy('medical_history_field_id')->map(function ($fields) {
            ...
            return [
                ...
                'value' => $this->valueFor($field->medical_history_field_id, $type),
            ];
        })->values()->all() : [];
